...
const MOCK_TOP_POSTS = [
  { id: 1, content: "Excited to announce our Series B funding! 🚀 #startup #growth", platform: 'linkedin', impressions: '45.2K', engagement: '3.8K', ctr: '12.5%', date: 'Oct 12' },
  { id: 2, content: "5 ways to improve your workflow today 🧵", platform: 'twitter', impressions: '12.1K', engagement: '850', ctr: '5.2%', date: 'Oct 15' },
  { id: 3, content: "Summer vibes at the office ☀️🍕", platform: 'instagram', impressions: '28.4K', engagement: '2.1K', ctr: '8.1%', date: 'Oct 10' },
  { id: 4, content: "New Tutorial: Getting Started with AI", platform: 'youtube', impressions: '15.6K', engagement: '1.2K', ctr: '15.3%', date: 'Oct 18' },
];

const MOCK_REPORTS: Report[] = [
  { id: '1', name: 'October Performance Summary', dateRange: 'Oct 1 - Oct 31, 2023', createdAt: '2 days ago', status: 'ready', format: 'pdf' },
  { id: '2', name: 'Q3 Executive Review', dateRange: 'Jul 1 - Sep 30, 2023', createdAt: '1 month ago', status: 'ready', format: 'pdf' },
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg flex flex-col justify-between">
                 <div>
                    ...
                    <p className="text-indigo-100 text-sm opacity-90 mb-6">Create professional, white-labeled reports for your clients or team in seconds.</p>
                 </div>
                 <button 
              ...
              <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
                 <div className="flex items-center justify-between mb-6">
                    <h3 className="font-bold text-lg text-slate-900 dark:text-white">Report History</h3>
                       ...
                       <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                          {reports.map((report) => (
                             <tr key={report.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="py-4 text-sm text-slate-600 dark:text-slate-400">{report.dateRange}</td>
                                <td className="py-4 text-sm text-slate-500 dark:text-slate-400">{report.createdAt}</td>
                                <td className="py-4">
                                   {report.status === 'ready' ? (
                                ...
                                <td className="py-4 text-right pr-2">
                                   <button 
                                     ...
                                     onClick={() => showToast(`Downloading ${report.format.toUpperCase()}...`, 'success')}
                                   >
                                      Download
                                   </button>
                                </td>
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
               <div className="p-6 border-b border-slate-100 dark:border-slate-800">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Top Performing Content</h3>
               </div>
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     <thead>
                           ...
                           <tr key={post.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                              <td className="py-4 pl-6 max-w-xs">
                                 <div className="flex flex-col">
                                    <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{post.content}</p>
                                    <span className="text-xs text-slate-500 dark:text-slate-400">{post.date}</span>
                                 </div>
                              </td>
                              <td className="py-4">
                                 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
                                    post.platform === 'linkedin' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' :
                                    post.platform === 'twitter' ? 'bg-sky-100 dark:bg-sky-900/30 text-sky-800 dark:text-sky-300' :
                                    post.platform === 'instagram' ? 'bg-pink-100 dark:bg-pink-900/30 text-pink-800 dark:text-pink-300' :
               ...
               <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/30 text-center">
                  <button className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:underline">View Full Analytics</button>
               </div>
            </div>
        </div>
      ) : (
        <div className="space-y-8 animate-in fade-in duration-300 relative">
             ...
             {userPlan === 'free' && (
                <div className="absolute inset-0 z-20 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-8 rounded-2xl">
                   ...
                   <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Competitor Analysis Locked</h3>
                   <p className="text-slate-600 dark:text-slate-400 max-w-md mb-6">Upgrade to Pro to track competitor performance, benchmark your growth, and spy on their best content.</p>
                   <button onClick={onOpenUpgrade} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/30">Upgrade to Pro</button>
                </div>
             )}
