...
import { X, Bell, CheckCircle2, AlertTriangle, Info, MessageSquare } from 'lucide-react';
...
const MOCK_NOTIFICATIONS: Notification[] = [
  { id: '1', type: 'mention', title: 'New Mention', message: 'Sarah mentioned you in "Summer Campaign"', timestamp: '10m ago', read: false },
  { id: '2', type: 'success', title: 'Post Published', message: 'Your post to LinkedIn was published successfully', timestamp: '1h ago', read: false },
  { id: '3', type: 'info', title: 'System Update', message: 'SocialFlow AI has been updated to version 2.0', timestamp: '1d ago', read: true },
  { id: '4', type: 'warning', title: 'Connection Lost', message: 'Please reconnect your Instagram account', timestamp: '2d ago', read: true },
];
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-slate-900 shadow-2xl z-[70] border-l border-slate-200 dark:border-slate-800 transform transition-transform duration-300 ease-in-out animate-in slide-in-from-right">
        <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 text-xs font-bold px-2 py-0.5 rounded-full">2 New</span>
          </div>
            ...
            >
              <div className="flex gap-3">
                <div className={`mt-1 p-2 rounded-lg h-fit ${
                   notif.type === 'mention' ? 'bg-indigo-100 dark:bg-indigo-900/30' :
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className={`text-sm font-semibold ${!notif.read ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-300'}`}>{notif.title}</h3>
                  ...
                  {!notif.read && (
                     <div className="mt-2">
                        <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline">Mark as read</span>
                     </div>
                  )}
                </div>
              </div>
