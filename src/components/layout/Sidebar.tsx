...
const AGENCY_WORKSPACES: Workspace[] = [
  { id: "1", name: "SocialFlow Agency", type: "agency", members: 5 },
  { id: "2", name: "Client: TechCorp", type: "team", members: 3 },
  { id: "3", name: "Client: GreenFoods", type: "team", members: 2 },
];

const PERSONAL_WORKSPACE: Workspace[] = [
  { id: "1", name: "My Workspace", type: "personal", members: 1 },
];
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const hasCustomLogo =
    branding.logoUrl && branding.logoUrl.length > 0 && userPlan === "agency";

  // Credits Logic
  const totalCredits =
    userPlan === "free" ? 10 : userPlan === "pro" ? 100 : Infinity;
  const usedCredits = 8; // Mock usage
  const creditsPercent =
    totalCredits === Infinity ? 100 : (usedCredits / totalCredits) * 100;
    ...
    <div className="w-64 h-full bg-slate-900 text-white flex flex-col border-r border-slate-800 shadow-xl">
      ...
      <div className="p-4 border-b border-slate-800 relative z-20">
        ...
        >
          ...
          {!isSingleUser && (
            <ChevronDown
              className={`w-4 h-4 text-slate-500 transition-transform shrink-0 ${isWorkspaceDropdownOpen ? "rotate-180" : ""}`}
            />
          )}
        </button>
      ...
      <div className="p-4 border-t border-slate-800 space-y-4">
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700">
          ...
          {userPlan === "free" && (
            <button
              ...
              className="w-full py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-colors shadow-sm flex items-center justify-center"
            >
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700 flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-indigo-400 truncate uppercase font-bold">
              {userPlan} Plan
            </p>
          </div>
          <button
