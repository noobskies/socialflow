...
import { AGENCY_WORKSPACES, PERSONAL_WORKSPACE } from '@/utils/constants';
            ...
            <div className="max-h-60 overflow-y-auto">
              {workspaces.map((ws) => (
                ...
                >
                  <span
                    className={
                      ws.id === activeWorkspace.id ? 'text-white font-medium' : 'text-slate-300'
                    }
                  >
                    {ws.name}
                  </span>
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs text-slate-400">
              {userPlan === 'agency' ? 'Unlimited' : `${usedCredits}/${totalCredits}`}
            </span>
          </div>
          {userPlan !== 'agency' && (
