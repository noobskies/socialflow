...
const Library: React.FC<LibraryProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'library' | 'rss' | 'buckets' | 'stock' | 'hashtags'>(
    'library'
  );
  ...
  const [hashtags, setHashtags] = useState<HashtagGroup[]>(MOCK_HASHTAG_GROUPS);
  ...
  const handleAutoSchedule = (bucket: Bucket) => {
    ...
    // Simulate creating posts with static IDs
    const baseId = `bucket-${bucket.id}-${Math.random().toString(36).substr(2, 9)}`;
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm mb-6 flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
              <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
                {['all', 'image', 'video', 'template'].map((type) => (
                  <button
                    key={type}
                    onClick={() => setActiveFilter(type as 'all' | 'image' | 'video' | 'template')}
