...
import { MOCK_INTEGRATIONS, MOCK_WORKFLOWS } from '@/utils/constants';

// Mock Data removed (imported from constants)
      ...
      {activeTab === 'workflows' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
          ...
          <div className="lg:col-span-2 space-y-6">
            {workflows.map((workflow) => (
              <div
                key={workflow.id}
                className={`bg-white dark:bg-slate-900 p-6 rounded-xl border transition-all duration-200 ${workflow.active ? 'border-indigo-200 dark:border-indigo-900/50 shadow-sm' : 'border-slate-200 dark:border-slate-800 opacity-75'}`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-start space-x-4">
                    <div
                      className={`p-3 rounded-lg ${workflow.active ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}
                    >
                  ...
                  <div className="flex flex-col items-end gap-3">
                    ...
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${workflow.active ? 'translate-x-6' : 'translate-x-1'}`}
                      />
                    </button>
                    {workflow.active && (
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 text-white shadow-lg">
              ...
              <p className="text-indigo-100 text-sm mb-4">
                Tell me about your business, and I'll design the perfect automation strategy.
              </p>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              <div className="space-y-3">
                {['RSS to Twitter', 'Instagram to Pinterest', 'Welcome New Followers'].map(
                  (t, i) => (
                    <div
                      key={i}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-300">
          {integrations.map((integration) => (
            <div
              key={integration.id}
              className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center text-center relative overflow-hidden group hover:shadow-md transition-all"
            >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${integration.connected ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}
              >
