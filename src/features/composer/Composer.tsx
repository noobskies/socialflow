...
import { MOCK_PRODUCTS, AI_TEMPLATES, MOCK_HASHTAG_GROUPS, TIMEZONES } from '@/utils/constants';
  ...
  const handleScheduleSubmit = () => {
    ...
    if (onPostCreated) {
      const newPost: Post = {
        ...
        platformOptions,
        poll: isPollActive
          ? { options: pollOptions.filter((o) => o.trim() !== ''), duration: pollDuration }
          : undefined,
      };
    ...
    showToast(
      `Post scheduled for ${scheduleDate} at ${scheduleTime} (${scheduleTimezone})${recurringMsg}`,
      'success'
    );
  ...
  const changeStatus = (newStatus: typeof workflowStatus) => {
    ...
    switch (newStatus) {
      ...
      case 'approved':
        ...
        break;
      case 'rejected':
        msg = 'Post rejected. Check comments.';
        break;
      case 'draft':
                ...
                <div className="text-center">
                  ...
                  <div
                    className={`text-lg font-bold px-3 py-1 rounded-full ${
                      analysisResult.engagementPrediction === 'High'
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full uppercase font-bold border ${
                  workflowStatus === 'approved'
                    ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 border-emerald-200'
                    : workflowStatus === 'rejected'
                      ? 'bg-rose-50 dark:bg-rose-900/20 text-rose-700 border-rose-200'
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setPlatformOptions({
                          ...platformOptions,
                          instagram: { ...platformOptions.instagram, firstComment: e.target.value },
                        })
                      }
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 px-4 py-3 border-b border-indigo-100 dark:border-indigo-900/30 flex justify-between items-center">
              <div className="flex items-center space-x-2 text-indigo-900 dark:text-indigo-200">
                ...
                <span className="font-semibold text-sm">
                  {activeTab === 'team' ? 'Team Collaboration' : 'Gemini 3 Pro Assistant'}
                </span>
              </div>
                      ...
                      {repurposedContent ? (
                        <div className="grid grid-cols-1 gap-3 mt-2">
                          <div
                            ...
                            onClick={() =>
                              applyRepurposed(repurposedContent.twitter.join('\n\n'), 'twitter')
                            }
                          >
                          ...
                          <div
                            className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-indigo-100 dark:border-indigo-900 cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-700 group"
                            onClick={() => applyRepurposed(repurposedContent.linkedin, 'linkedin')}
                          >
                            <div className="flex justify-between items-center mb-1">
              ...
              <div className="px-6 pb-4 relative z-10">
                <div className="relative inline-block group/media">
                  ...
                  <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover/media:opacity-100 transition-opacity">
                    ...
                    <button
                      onClick={() =>
                        mediaType === 'video' ? setIsEditingVideo(true) : setIsEditingImage(true)
                      }
                      className="bg-white dark:bg-slate-800 text-slate-500 hover:text-indigo-500 rounded-full p-1 shadow-md border border-slate-100 dark:border-slate-700"
                      title="Edit Media"
                    >
