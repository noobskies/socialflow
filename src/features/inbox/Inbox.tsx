...
const Inbox: React.FC<InboxProps> = ({ showToast }) => {
  ...
  const getPlatformIcon = (platform: Platform) => {
    switch (platform) {
      case 'twitter':
        return <Twitter className="w-3 h-3 text-white" />;
      case 'linkedin':
      ...
      case 'pinterest':
        return <Pin className="w-3 h-3 text-white" />;
    }
  };
          ...
          {activeTab === 'messages' ? (
              ...
              >
                <div className="flex gap-3">
                  <div className="relative shrink-0">
                    ...
                    />
                    <div
                      className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center ${getPlatformColor(msg.platform)}`}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                      <span
                        className={`font-semibold text-sm truncate ${msg.unread ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}
                      >
                        {msg.author}
                      </span>
                    ...
                    <p
                      className={`text-sm line-clamp-2 ${msg.unread ? 'text-slate-800 dark:text-slate-200 font-medium' : 'text-slate-500 dark:text-slate-500'}`}
                    >
                      {msg.content}
                    </p>
                  </div>
            ...
            <div className="p-2 space-y-2">
              ...
              {MOCK_LISTENING.map((item) => (
                <div
                  key={item.id}
                  className="p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors cursor-pointer group"
                >
        ...
        {activeTab === 'messages' && selectedMessage ? (
                ...
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    <span className="text-xs font-normal text-slate-500 dark:text-slate-400">
                      {selectedMessage.authorHandle}
                    </span>
                  </h3>
                  <div className="flex items-center gap-2">
                    <div
                      className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-white uppercase ${getPlatformColor(selectedMessage.platform)}`}
                    >
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">
                        We are excited to announce our Series B funding round led by...
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-600 p-8 text-center">
            ...
            <p className="max-w-md text-sm">
              Select a mention on the left to view details, analyze sentiment, and engage directly
              from here. Tracking 3 keywords.
            </p>
          </div>
        ) : (
